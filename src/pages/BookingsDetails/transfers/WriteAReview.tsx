...
const WriteAReview = ({ closeModal, bookings }: props) => {
  ...
  const transfer = bookings?.transfers?.[0];
  const listingName = bookings?.listing_name || "Transfer";
  const pickupDate = transfer?.pickupInformation?.date;
  const currency = bookings?.currency ?? "";
  const priceLabel = currency ? `${currency} ` : "";
  const priceText =
    bookings?.totalAmount ?? bookings?.totalNetAmount ?? "-----";
  ...
  const submitAReview = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await submitBookingReview(reference, {
        overallRating,
          ...
          {[1, 2, 3, 4, 5].map((star) => (
            ...
            >
              <svg
                ...
                stroke={star <= ratings[category] ? "#FBBF24" : "#9CA3AF"}
                strokeWidth="2"
                strokeLinecap="round"
        ...
        >
          ...
          <div className="flex items-start justify-normal gap-4 px-6 border-b border-gray-300 py-4">
            ...
            <div className="flex flex-col justify-between h-full">
              <h3 className="font-semibold text-gray-800">{listingName}</h3>
              <p className="text-gray-500 text-sm">
                {pickupDate && new Date(pickupDate).toDateString()}
              </p>
          ...
          <div className="px-6 py-4 space-y-5">
            ...
            <div className="space-y-4">
              <StarRating label="Punctuality" category="punctuality" />
              <StarRating label="Vehicle condition" category="vehicle_condition" />
              <StarRating label="Professionalism" category="professionalism" />
              <StarRating label="Safety" category="safety" />
            </div>
            ...
            <div className="space-y-2 mt-4">
              ...
              <p className="text-xs text-gray-400">
                Your review helps us improve and assists other travellers in
                making informed decisions. Thank you for sharing your
                experience!
              </p>
            </div>
          </div>
