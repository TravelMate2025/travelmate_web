    ...
    <div>
      {bookings?.length === 0 ? (
        <EmptyState
          ...
          content="You haven't completed any bookings yet. When you do, they will appear here."
        />
      ) : (
        bookings.map((item) => {
          ...
          return (
            <div
              ...
              onClick={() => {
                if (item.type === "stay") {
                  navigate(
                    `/bookings/stays-details/?booking_reference=${encodeURIComponent(item.reference)}&session_id=${encodeURIComponent(item.session_id)}&booking_status=completed`,
                  );
